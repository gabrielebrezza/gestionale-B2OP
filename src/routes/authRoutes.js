...
router.get('/resetPassword/:id', authenticateJWT, async (req, res) =>{
    try {
        ...
        if(!user){
            return res.render('errorPage', { err: 'Token non valido' });
        }
        if(user.isPasswordResetExpired()){
            await noleggiatori.findOneAndUpdate({ "_id": id }, {$unset: {"resetPassword": ""}});
            return res.render('errorPage', { err: 'Token scaduto' });
        }
        if (!await bcrypt.compare(token, user.resetPassword.token)) return res.render('errorPage', { err: 'Token errato' });

        res.render('user/resetPassword', {token, id, customerId: false});
    } catch (error) {
        console.log(`Errore nella modifica della password: ${error}`)
        return res.render('errorPage', { err: 'Errore del server' });
    } 
});
...
router.post('/admin/approvation', authenticateJWT, async (req, res) =>{
    ...
    const text = `Gentile ${admin.nome} ${admin.cognome}, ti informiamo che il tuo account Admin è stato approvato. Per accedere ${process.env.SERVER_URL}/admin`;
    try {
        const result = await sendEmail(email, subject, text);
        console.log(result)
    } catch (error) {
...
router.post('/user/forgotPassword', async (req, res) => {
    try {
        ...
        const user = await noleggiatori.findOne({"contatti.email": email});
        if(!user || !user.password) return res.status(404).json({success: false, dataNotExist: true});

        const token = crypto.randomBytes(16).toString('hex').slice(0, 16);
        await noleggiatori.findOneAndUpdate({"contatti.email": email}, {"resetPassword.token": await bcrypt.hash(token, 10), "resetPassword.createdAt": Date.now()});
        const subject = 'Password Reset';
        const text = `Gentile ${user.nome} ${user.cognome}, per cambiare la password ${process.env.SERVER_URL}/resetPassword/${encodeURIComponent(user._id)}?t=${encodeURIComponent(token)}, se non sei stato tu a richiedere il reset della password ignora pure questa email.`;
        try {
            const result = await sendEmail(email, subject, text);
            console.log(result)
        } catch (error) {
        ...
        console.log('errore durante il reset della password');
...
router.post('/resetPassword/verify', async (req, res) => {
    try {
        ...
        const user = await noleggiatori.findOne({"_id": id});
        if(!user || !user.password) return res.render('errorPage', { err: 'Errore nel reset della password' });

        if(user.isPasswordResetExpired()){
            await noleggiatori.findOneAndUpdate({ "_id": customerId }, {$unset: {"resetPassword": ""}});
            return res.render('errorPage', { err: 'Token scaduto' });
        }
        if (!await bcrypt.compare(token, user.resetPassword.token)) return res.render('errorPage', { err: 'Token errato' });

        await noleggiatori.findOneAndUpdate({"_id": id}, {"password": await bcrypt.hash(password, 10), $unset: {"resetPassword": ""}});
